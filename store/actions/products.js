  ...
  return async (dispatch, getState) => {
    ...
    try {
      const response = await fetch(
        "https://shop-app-bf402.firebaseio.com/products.json"
      );
...
export const deleteProduct = productId => {
  return async (dispatch, getState) => {
    ...
    const response = await fetch(
      `https://shop-app-bf402.firebaseio.com/products/${productId}.json?auth=${token}`,
      {
        method: "DELETE"
      }
    );
...
export const createProduct = (title, description, imageUrl, price) => {
  ...
  return async (dispatch, getState) => {
    ...
    const userId = getState().auth.userId;
    const response = await fetch(
      `https://shop-app-bf402.firebaseio.com/products.json?auth=${token}`,
      {
        method: "POST",
        headers: {
...
export const updateProduct = (id, title, description, imageUrl) => {
  return async (dispatch, getState) => {
    ...
    const response = await fetch(
      `https://shop-app-bf402.firebaseio.com/products/${id}.json?auth=${token}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json"
